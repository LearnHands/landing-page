...
const fetchWithFallback = async (endpoint) => {
  try {
    ...
    console.error(`[API Error] Error al consultar ${endpoint}:`, err.message);
    throw err;
  }
};
...
export default function DashboardSection() {
  ...
  const [dashboardTab, setDashboardTab] = useState('students');
  ...
  const fetchClasses = async () => {
    try {
      const dataClasses = await fetchWithFallback('/api/teacher/classes');
      setClassesList(dataClasses);
    } catch (err) {
      console.warn('[Dashboard] No se pudo cargar la lista de clases:', err.message);
    }
  };

  const fetchData = async (classCode = selectedClass) => {
    setLoading(true);
    setError(null);
    try {
      // Fetch list of students for class
      const studentsEndpoint = classCode === 'ALL'
        ? '/api/teacher/students'
        : `/api/teacher/students?class_code=${classCode}`;
      const dataStudents = await fetchWithFallback(studentsEndpoint);
      ...
      if (classCode !== 'ALL') {
        try {
          const dataClass = await fetchWithFallback(`/api/teacher/class-info?code=${classCode}`);
  ...
  const handleRegenCode = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        fetchClasses(); // Refresh class list to update code
      }
    } catch (err) {
  ...
  const classMetrics = useMemo(() => {
    ...
    return metrics.filter(m => studentUsernames.has(m.username));
  }, [metrics, students]);

  const filteredMetrics = useMemo(() => {
    if (studentFilter === 'ALL') return classMetrics;
    return classMetrics.filter(m => m.username === studentFilter);
  }, [classMetrics, studentFilter]);
        ...
        <div className="flex flex-wrap items-center gap-3">
          {classesList.length > 0 && (
            <select
              ...
              className="px-4 py-3 glass rounded-2xl border border-white/10 bg-[#03030b]/60 text-white text-[10px] font-black uppercase tracking-widest focus:outline-none focus:border-purple-500/50 cursor-pointer"
            >
              <option value="ALL" className="bg-[#03030b] text-white">Todas las Clases</option>
              {classesList.map(c => (
                <option key={c.class_code} value={c.class_code} className="bg-[#03030b] text-white">
        ...
        <div className="space-y-8">
          ...
          <div className="glass p-5 rounded-3xl border border-white/5 flex flex-wrap items-center gap-4 bg-black/20">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              >
                Todos ({classMetrics.length})
              </button>
              {studentList.map(s => {
                const count = classMetrics.filter(m => m.username === s.username).length;
                return (
                  <button
                    key={s.username}
                    onClick={() => setStudentFilter(s.username)}
                    className={`px-4 py-2 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${
                      studentFilter === s.username
                        ? 'bg-purple-500/20 border-purple-500/50 text-purple-300'
                        : 'bg-white/5 border-white/5 text-white/40 hover:text-white hover:border-white/20'
                    }`}
                  >
          ...
          <div className="flex flex-wrap border-b border-white/10 gap-8">
            {[
              { id: 'students', label: `Estudiantes (${students.length})` },
              ...
              {dashboardTab === 'students' && (
                <div className="glass p-6 rounded-[32px] border border-white/10 bg-black/40 flex flex-col gap-6">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-base font-bold uppercase tracking-tight text-white/90">Lista de Estudiantes</h3>
                      <p className="text-white/40 text-[10px] uppercase font-black tracking-widest mt-1">
                        {selectedClass === 'ALL' ? 'Todos los estudiantes registrados' : `Estudiantes en la clase seleccionada`}
                      </p>
                    </div>
                    <span className="px-3.5 py-1.5 bg-indigo-500/10 border border-indigo-500/20 rounded-full text-[9px] font-black uppercase tracking-wider text-indigo-400">
                  ...
                  {students.length === 0 ? (
                    <div className="py-16 flex flex-col items-center gap-4 text-center">
                      ...
                      <div>
                        <h4 className="text-lg font-black uppercase text-white/50">Sin estudiantes registrados</h4>
                        <p className="text-white/25 text-xs mt-1 max-w-xs">Comparte el código de clase con tus alumnos para que puedan unirse.</p>
                      </div>
                    </div>
                  ) : (
                      ...
                      <table className="w-full text-left text-[11px] border-collapse">
                        <thead>
                          <tr className="border-b border-white/10 text-white/30 uppercase tracking-widest font-black">
                            ...
                            <th className="pb-3 text-center">Cédula</th>
                            <th className="pb-3 text-center">Partidas</th>
                            <th className="pb-3 text-center">Puntaje Total</th>
                            <th className="pb-3 text-right">Último Juego</th>
                          </tr>
                        </thead>
                        <tbody className="text-white/80 font-bold">
                          {students.map((s, idx) => {
                            const sessionCount = classMetrics.filter(m => m.username === s.username).length;
                            return (
                              <tr key={s.username} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-all">
                                <td className="py-3.5 text-white/25 font-medium">{idx + 1}</td>
                                <td className="py-3.5">
                                  <div className="flex flex-col">
                                    <span className="font-bold text-white">{s.display_name || s.username}</span>
                                    {s.display_name && s.display_name !== s.username && (
                                      <span className="text-[9px] text-white/30 font-medium tracking-normal">{s.username}</span>
                                    )}
                                  </div>
                                </td>
                                <td className="py-3.5 text-center">
                                  <span className="font-mono text-[10px] text-white/40 bg-white/5 px-2 py-1 rounded-lg">{s.username}</span>
                                </td>
                                <td className="py-3.5 text-center text-purple-400">{sessionCount}</td>
                                <td className="py-3.5 text-center text-amber-400 font-display italic">
                                  {Number(s.total_score || 0)} pts
                                </td>
                                <td className="py-3.5 text-right text-white/30">
                                  {s.last_played_at
                                    ? new Date(s.last_played_at).toLocaleString('es-ES', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })
                                    : <span className="text-white/20">Sin actividad</span>}
                                </td>
                              </tr>
                            );
                          })}
              ...
              <table className="w-full text-left text-[11px] border-collapse">
                ...
                <tbody className="text-white/80 font-bold">
                  {filteredMetrics.slice(0, 5).map((m, idx) => {
                    const studentObj = students.find(s => s.username === m.username);
                    const displayName = studentObj ? (studentObj.display_name || studentObj.username) : m.username;
                    const hasDifferentName = studentObj && studentObj.display_name && studentObj.display_name !== studentObj.username;
                    return (
                      <tr key={idx} className="border-b border-white/5 last:border-0 hover:bg-white/5 transition-all">
                        <td className="py-3.5 uppercase text-purple-400 tracking-wider">
                          <div className="flex flex-col text-left">
                            ...
                            {hasDifferentName && (
                              <span className="text-[8px] text-white/30 pl-3.5 font-medium tracking-normal lowercase">({m.username})</span>
                            )}
                          </div>
                        </td>
